...
def train_model(model, train: Tuple[csr_matrix, np.ndarray], test: Tuple[csr_matrix, np.ndarray],
    ...
    for _ in display:
        ...
        for batch_n in inner_loop:
            ...
            inner_loop.set_description_str(f'Train Loss: {train_loss}')
        train_losses_epochs.append(train_loss)

    return train_losses_epochs
...
def main():
    ...
    dnn = DNN(layer_sizes=[INPUT_SIZE, 256, 256, 256, 1])
    # dnn = DNN(layer_sizes=[X_train.shape[1], 256, 1])

    train_losses = train_model(model=dnn, train=(X_train, Y_train), test=(X_test, Y_test), batch_size=128,
                                epochs=5)
    print(train_losses)
